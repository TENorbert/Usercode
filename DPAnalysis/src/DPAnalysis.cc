...
// $Id: DPAnalysis.cc,v 1.2 2012/04/19 19:35:15 sckao Exp $
//
//
...
//    }
   ...
   if ( !isData ) gen->GetGen( iEvent, leaves );   //  Call MC here!
...
bool DPAnalysis::EventSelection(const edm::Event& iEvent ) {
   ...
   */
   ...
   if ( selectedPhotons.size() > 0 ) {
	 if ( selectedPhotons[0]->pt() < 65. ) passEvent = false ;  //   choose New Cut for Photons with pt > 65 GeV
   }
...
bool DPAnalysis::PhotonSelection( Handle<reco::PhotonCollection> photons, vector<const reco::Photon*>& selectedPhotons ) {
   ...
   for(reco::PhotonCollection::const_iterator it = photons->begin(); it != photons->end(); it++) {
       // fiducial cuts
	 if ( it->pt() < photonCuts[0] || fabs( it->eta() ) > photonCuts[1] ) continue ;   // Accepting only Photons in Eta < 2.4 Low EE
